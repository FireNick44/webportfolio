...
- Structural/tuning knobs are **not** in the JSON (they are not daily-edited content). They live in `src/physics/constants.ts` as named constants:
  ...
  - `AREA_PER_FLASK` (the single density knob, px² per flask)
  - `MIN_FLASKS`, `MAX_FLASKS` (clamp)
  - `MAX_PHYSICS_FLASKS` (body budget)
  - `MIN_FLASK_SPACING_PX` (replaces `MIN_SAME_LAYER_DISTANCE_PCT`)
...
1. **Target count:** `targetCount = clamp(round((width * height) / AREA_PER_FLASK), MIN_FLASKS, MAX_FLASKS)`. Holds density constant across viewports. `AREA_PER_FLASK` is tuned visually once.
...
5. **Horizontal placement:** seeded scatter within each band using a **pixel** minimum spacing (`MIN_FLASK_SPACING_PX`) and rejection sampling — consistent gaps at any width. (Replaces the percentage-based spacing.)
...
Mobile (`width < MOBILE_BREAKPOINT`) keeps a reduced-count variant of the same algorithm (smaller `targetCount`, fewer/no skeleton bands as appropriate), not a separate code path where avoidable.
...
- **Quiescence guard (the cure for "never-ending physics").** Track whether any dynamic body is awake. When all sleep, **stop calling `Engine.update`** and stop the sync loop. Resume on any wake event: mouse bump, drag start, scroll nudge, resize. Idle page = 0 physics/render CPU.
...
- **Desktop:** a small cursor collision body (radius ~7, down from the dormant `MOUSE_BODY_RADIUS = 15`) follows the pointer (wires up the existing-but-unused `CAT_MOUSE` / `MOUSE_MASK` scaffolding). On collision with a flask, apply a **one-shot horizontal impulse** ("nuke") that knocks it into an L↔R shake; the chain + damping resolve it back to rest. Click-and-drag is retained alongside.
...
`AREA_PER_FLASK`, `MIN_FLASK_SPACING_PX`, `MOUSE_BODY_RADIUS`, shake impulse magnitude, `SHAKE_COOLDOWN_MS`, `friction`, `sleepThreshold`, `MAX_PHYSICS_FLASKS`. These are expected to be adjusted visually after the system works; the design fixes the *structure*, not the final numeric feel.
