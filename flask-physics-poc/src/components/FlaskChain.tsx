...
export default function FlaskChain({
  ...
  useEffect(() => {
    ...
    if (flaskEl) {
      ...
      // Compensate for scale shrinking the flask toward its center —
      // without this the visual top drifts down, creating a gap.
      const scaleOffsetY = (FLASK_HEIGHT / 2) * (1 - scale);
      flaskEl.style.transform = `translate(${x}px, ${currentY - scaleOffsetY}px) scale(${scale})`;
  ...
  const syncDom = useCallback(() => {
    ...
    if (flaskEl) {
      ...
      // Compensate for scale shrinking flask toward center — close the gap
      const scaleOffsetY = (FLASK_HEIGHT / 2) * (1 - scale);
      const y = fb.position.y - FLASK_HEIGHT / 2 - scaleOffsetY;
