...
export default function Footer({ dict }: { dict: Dictionary; lang: string }) {
  ...
  return (
    <footer className="relative isolate overflow-hidden text-[#f5f0e6]">
      {/* Darker "ocean" bubbles ending — reuses the hero bubbles, dimmed, with a
          wave flowing in from the section above and a colourful sand ground. */}
      <div aria-hidden className="absolute inset-0 -z-10">
        ...
        />
        {/* Always-dark wash so the bubbles read dark + the light text stays
            legible in BOTH themes (matches the original's dark "bye" ending). */}
        <div className="absolute inset-0 bg-black/60" />
        {/* Short blend at the very top so it flows out of the section above. */}
        <div className="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-background to-transparent" />
      </div>

      {/* wave transition from the previous section into the bubbles */}
      <WaveDivider
        fill="var(--background)"
        flip
      ...
      {/* colourful layered sand "ground" at the very bottom */}
